...
class XetraSourceConfig(NamedTuple):
    ...
    src_first_extract_date: determines the date on which we start extracting the source
...
class XetraTargetConfig(NamedTuple):
    ...
    trg_format: file format of the target file (.parquet)
    """
...
class XetraETL():
    ...
    def __init__(self, s3_bucket_src: S3BucketConnector,
        ...
        :param s3_bucket_trg: connection to source S3 bucket
    ...
    def extract(self):
        ...
        files = [key for date in self.extract_date_list\
                 for key in self.s3_bucket_src.list_files_in_prefix(f'{date}')]
        ...
        self._logger.info('Extracting Xetra source file finished.')
        return data_frame
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        :params data_frame: Pandas DataFrame as Input

        :returns:
            data_frame: Transformed Pandas DataFrame as Input
        """
        if data_frame.empty:
        ...
        data_frame.loc[:, self.src_args.src_columns]
        # Removing rows with missing values
        data_frame.dropna(inplace=True)
        # Calculating opening price per ISIN and day 
        # (same ISIN same day has 24 same open price entries)
        ...
        # last start price = last end price of a day per ISIN
        ...
        # Aggregating per ISIN and day -> opening price and closing price
        ...
        data_frame = data_frame.groupby([
            ...
            .agg({
                ...
                self.trg_args.trg_col_clos_price: 'min',  # they are all the same number
        ...
        # Removing the date before extract date
        data_frame =  data_frame[data_frame[self.src_args.src_col_date] >= self.extract_date]\
    ...
    def load(self, data_frame: pd.DataFrame):
        ...
        :param data_frame: Pandas DataFrame as an Input  
        """
        # Creating target key
        target_key = (
