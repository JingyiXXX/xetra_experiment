...
class S3BucketConnector():
    ...
    def __init__(self, access_key: str, secret_key: str,
        ...
        :param screct_key: secret key for accessing S3
        :param endpoint_url: end point url to S3
        :param bucket: S3 bucket name
        """
        # using the name of s3.py
        ...
        self._s3 = self.session.resource(service_name='s3', endpoint_url=self._endpoint_url)
    ...
    def list_files_in_prefix(self, prefix: str):
        ...
        :param prefix: prefix of the files on the S3 bucket that should be filtered

        returns:
            files: list of all the file names containing all the prefix
        """
    ...
    def read_csv_to_df(self, key: str, decoding = 'utf-8', sep = ','):
        ...
        :param decoding: decoding the data inside the csv file
        :param sep: separater of the csv file
    ...
    def write_df_to_s3(self, data_frame: pd.DataFrame, key: str, file_format: str):
        ...
        if data_frame.empty:
            ...
            return None
        if file_format == S3FileTypes.CSV.value: # attribute CSV can be used without instantiating
            out_buffer = StringIO()
            data_frame.to_csv(out_buffer, index=False)
            return self.__put_object(out_buffer, Key=key)
    ...
    def __put_object(self, out_buffer: StringIO or BytesIO, Key: str):
        ...
        """
        self._logger.info('Writing the file to %s/%s/%s', self._endpoint_url, self._bucket.name, Key)
