...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Creating S3 access key as environment variables
    ...
    def test_update_meta_file_no_meta_file(self):
        ...
        """
        # Expected results
        date_list_exp = ['2022-04-16', '2022-04-17']
        proc_date_list_exp = [datetime.today().date()] * 2 # processed date when add the date_list into meta file
        # Test init
        meta_key = 'meta.csv'
        # Method execution
        MetaProcess.update_meta_file(date_list_exp, meta_key, self.s3_bucket_meta) # submit the expected date list
        # Read meta file
    ...
    def test_update_meta_file_empty_date_list(self):
        ...
        and there is no meta file in S3
        """
        # Expected results
        return_exp = True
    ...
    def test_update_meta_file_meta_file_wrong(self):
        ...
        date_list_old = ['2022-04-12', '2022-04-13']
        date_list_new = ['2022-04-16', '2022-04-17']
        exception_exp = WrongMetaFileException
        # Test init
        meta_key = 'meta.csv'
        wrong_col = 'wrong_column'
        meta_content = (
            f'{wrong_col},'
            f'{MetaProcessFormat.META_PROCESS_COL.value}\n'
            f'{date_list_old[0]},'
    ...
    def test_return_date_list_no_meta_file(self):
        ...
        """
        # Expected results
        # date_list_exp: [t-3, t-2, t-1, t]
        date_list_exp = [
            (datetime.today().date() - timedelta(days=x))\
                .strftime(MetaProcessFormat.META_DATE_FORMAT.value) for x in range(4)
        ]
        # min_date_exp = t-2 
    ...
    def test_return_date_list_meta_file_ok(self):
        ...
        date_list_exp = [
            [(datetime.today().date() - timedelta(days=x))\
                .strftime(MetaProcessFormat.META_DATE_FORMAT.value) for x in range(3)],
            [(datetime.today().date() - timedelta(days=x))\
                .strftime(MetaProcessFormat.META_DATE_FORMAT.value) for x in range(4)],
            [(datetime.today().date() - timedelta(days=x))\
                .strftime(MetaProcessFormat.META_DATE_FORMAT.value) for x in range(9)],
        ...
        # Method execution
        # use a for loop to go through three scenarios
        for count, first_date in enumerate(first_date_list):
            min_date_return, date_list_return = MetaProcess.return_date_list(first_date,
                                                          meta_key, self.s3_bucket_meta)
            # Test after method execution
    ...
    def test_return_date_list_meta_file_wrong(self):
        ...
        # Test init
        meta_key = 'meta.csv'
        # Create a meta file with wrong column
        meta_content = (
