...
class TestS3BucketConnectorMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # create a Moto's 'virtual' AWS account
        self.mock_s3 = mock_s3()
        self.mock_s3.start()
        # Defining the class arguments (in total, 4 arguments for S3BucketConnector)
        self.s3_access_key = 'AWS_ACCESS_KEY_ID'
        self.s3_secret_key = 'AWS_SECRET_ACCESS_KEY'
        self.s3_endpoint_url = 'https://s3.ca-central-1.amazonaws.com'
        self.s3_bucket_name = 'test-bucket'

       # Creating S3 access keys as environemnt variables
        os.environ[self.s3_access_key] = 'KEY1' # doesn't matter what the value is for key as we only mock the s3
        os.environ[self.s3_secret_key] = 'KEY2'
        # Creating a bucket on the mocked s3
        self.s3 = boto3.resource(service_name='s3', endpoint_url=self.s3_endpoint_url)
        self.s3.create_bucket(Bucket=self.s3_bucket_name,
                              CreateBucketConfiguration={
                                'LocationConstraint': 'ca-central-1' # constraint is necessary, otherwise it throws error.
                              })
    ...
    def test_list_files_in_prefix_ok(self):
        ...
        self.assertEqual(len(list_result), 2) # check return list has two items
    ...
    def test_list_files_in_prefix_wrong_prefix(self):
        """
        Tests the lsit_files_in_prefix method in case of a
        wrong or not exsiting prefix
        """
        # Expected results
        prefix_exp = 'no-prefix/' # to test a prefix that does not exist
        # Method execution
        list_result = self.s3_bucket_conn.list_files_in_prefix(prefix=prefix_exp)
        # Tests after method execution
        self.assertEqual(len(list_result), 0) # check return list has two items
    ...
    def test_read_csv_to_df_ok(self):
        ...
        # Method execution
        with self.assertLogs() as logm:   # self.assertLogs(), a method of unittest class
            df_result = self.s3_bucket_conn.read_csv_to_df(key_exp)
            # Log test after method execution 
            self.assertIn(log_exp, logm.output[0]) # first log that is written
        # Test after method execution
        self.assertEqual(df_result.shape[0], 1)
        self.assertEqual(df_result.shape[1], 2)
        self.assertEqual(df_result.columns[0], col1_exp)
        self.assertEqual(df_result.columns[1], col2_exp)
    ...
    def test_write_df_to_s3_empty(self):
        ...
        with self.assertLogs() as logm:
            write_result = self.s3_bucket_conn.write_df_to_s3(df_empty, key_exp, file_format)
            # Log test after method execution
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
        self.assertEqual(return_exp, write_result)

    def test_write_df_to_s3_csv(self):
        """
        Tests the write_df_to_s3 method
        if writing csv is sucessful
        """
        # Expected results
        return_exp = True
        df_exp = pd.DataFrame([['A', 'B'], ['C', 'D']], columns=['col1', 'col2'])
        key_exp = 'test.csv'
        log_exp = f'Writing the file to {self.s3_endpoint_url}/{self.s3_bucket_name}/{key_exp}'
        # Test init
        file_format = 'csv'
        # Method execution
        with self.assertLogs() as logm:
            write_result = self.s3_bucket_conn.write_df_to_s3(df_exp, key_exp, file_format)
            # Log test after method execution
            self.assertIn(log_exp, logm.output[0])
        # Test after method execution
    ...
    def test_write_df_to_s3_wrong_format(self):
        """
        Tests the write_df_to_s3 method
        if a not support format is given as an argument
        """
        # Expected results
        df_exp = pd.DataFrame([['A', 'B'], ['C', 'D']], columns=['col1', 'col2'])
        key_exp = 'test.parquet'
        ...
        # Method execution
        with self.assertLogs() as logm:
            with self.assertRaises(exception_exp): # WrongFormatException will be raised, unit test provides assertRaises() method to check if it is raised.
                write_result = self.s3_bucket_conn.write_df_to_s3(df_exp, key_exp, format_exp)
                # Log test after method execution
                self.assertIn(log_exp, logm.output[0])
...
if __name__ == '__main__':
    unittest.main()  # unittest will execute sequentially: setUp, all other other methods, tearDown
    # equivalent to manually do the following:
